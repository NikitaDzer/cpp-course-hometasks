...
public:
    constexpr static bool value = std::is_same_v<T, current> || Has<T, rest>::value;
};
...
static_assert(has_v<int, Deque<double, char, int>> == true);
static_assert(has_v<int, Deque<double, char, float>> == false);

static_assert(has_v<std::byte, Deque<>> == false);
static_assert(has_v<std::byte, Deque<std::byte>> == true);
